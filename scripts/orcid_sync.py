...
LAB ONCE — Laboratorio de Otoneurología Clínica y Neurociencias del Equilibrio.

Copiado y adaptado tal cual del sincronizador en producción del Laboratorio
de Neurosistemas (mismo diseño, misma lógica). Ver ese repo y
GUIA-REPLICAR-SITIO-ACADEMICO.md si hace falta más contexto.

Qué hace
--------
1. Lee data/orcid.yaml y toma los ORCID iD configurados (hoy, solo el del
   director del laboratorio — ver la nota en ese archivo).
...
def sesion_http(correo: str) -> requests.Session:
    ...
    s.headers.update({
        ...
        "User-Agent": f"labonce-web/1.0 (https://labonce.cl; mailto:{correo})",
    })
    return s
...
def main() -> int:
    ...
    correo = op.get("correo_contacto") or "webmaster@labonce.cl"
    ...
    # NO truncar la cita: en estilo Vancouver los primeros ~90 caracteres son el
    # listado de autores y el título queda decapitado, con lo que el match por
    # título de más abajo nunca se cumple (el chequeo es `clave_t[:60] in h`,
    # un substring: una cita más larga solo puede ayudar, nunca dañar).
    titulos_hist = {normalizar(p.get("cita", "")) for p in historicas}
